...
def _klinear(args, output_size, bias, bias_start=0.0, scope=None):
    ...
    out_sqrt = int(np.sqrt(output_size))
    hundredth_total_arg_size = int(total_arg_size / 100)
    part_in1 = 100
    part_in2 = hundredth_total_arg_size
    part_out1 = out_sqrt
    part_out2 = out_sqrt

    with vs.variable_scope(scope or "Linear"):
        matrix1 = vs.get_variable(
            "Matrix1", [part_out1, part_in1], dtype=dtype)
        ...
        if len(args) == 1:
            ...
            res = gmatmul(inputX_reshaped, matrix2, reduce_dim=2)
            res = gmatmul(matrix1, res, reduce_dim=1)
            res = tf.reshape(res, (-1, output_size))

            # res = math_ops.matmul(input_tensor, matrix)
        if not bias:
            return res
        bias_term = vs.get_variable(
            "Bias", [output_size],
...
def _klinear_flat(args, output_size, bias, bias_start=0.0, scope=None):
    ...
    with vs.variable_scope(scope or "Linear"):
        ...
        if True:
            ...
            # res = gmatmul(matrix1, res, reduce_dim = 1)
            # res = tf.reduce_sum(res * tf.expand_dims(matrix1, 0), 1)
            # res = math_ops.matmul(input_tensor, matrix)
